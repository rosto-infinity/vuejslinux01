...
const deleteColor = (color) => {
  ...
  colors.value = colors.value.filter(
    // -Inclut uniquement les couleurs qui ne correspondent pas à celle à supprimer
    col => col !== color 
  )
}
...
const sortColors = () => {
  ...
  // -- Trie le tableau des couleurs sans mutation
  colors.value = colors.value.toSorted((a, b) =>  a > b ? 1 : -1)
}
